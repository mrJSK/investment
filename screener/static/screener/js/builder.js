...
// // --- End of builder.js ---
// --- Constants ---
let INDICATORS = []; // Will be fetched from backend
...
// Extra param fields (ensure these IDs exist in dashboard.html)
...
function fetchIndicatorsAndRender() {
  fetch("/api/indicators/") // Using your Django endpoint
    .then((response) => response.json())
    .then((data) => {
      if (data.indicators) {
        INDICATORS = data.indicators.map((ind) => ({
          value: ind.value, // e.g., "SMA"
          label: ind.label, // e.g., "Simple Moving Average (SMA)"
          // We need to fetch params for each indicator if they are not sent with the list
          // For now, let's assume some common params or fetch them on demand in openIndicatorConfigEditor
          // Or, your /api/indicators/ should ideally return params list for each indicator
          params: getImplicitParams(ind.value), // Heuristic or pre-defined
        }));
      } else {
        console.warn(
          "No indicators found from API, using default mock if any was defined."
        );
        // Fallback to a minimal set if API fails or returns empty (already in global scope)
        if (INDICATORS.length === 0) {
          // Check if it wasn't pre-populated by previous mock
          INDICATORS = [
            {
              value: "CLOSE",
              label: "Close Price",
      ...
      renderFilters(); // Initial render after fetching
    })
    .catch((error) => {
      console.error("Error fetching indicators:", error);
      // Use a default mock list if API fails
      if (INDICATORS.length === 0) {
        INDICATORS = [
          {
            value: "CLOSE",
...
// Helper to guess common params for mock (replace with actual param data from API if possible)
function getImplicitParams(indicatorValue) {
  const upperVal = indicatorValue.toUpperCase();
  let params = ["timeframe"];
  if (["SMA", "EMA", "RSI", "BBANDS"].includes(upperVal))
    params.push("field", "period");
  if (["MACD", "MACDEXT", "MACDFIX"].includes(upperVal))
    params.push("field", "fast_period", "slow_period", "signal_period");
  if (["STOCH", "STOCHF", "STOCHRSI"].includes(upperVal))
    params.push("timeframe", "fastk_period", "slowk_period", "slowd_period"); // STOCH uses different param names
  if (upperVal === "SUPERTREND") params.push("period", "multiplier");
  // Add more common ones or rely on a more detailed API response
  return params;
}
...
function createFilterRowElement(filter, filterIndex) {
  ...
  if (filter.left)
    ...
    rowDiv.appendChild(
      createButtonToken("📊 Select Left", "builder-token", () =>
        openIndicatorPickerModal(filterIndex, "left")
      )
    );
  ...
  if (filter.op) {
    ...
    rowDiv.appendChild(
      createButtonToken(`${opMeta.icon} ${opMeta.label}`, "token-op", () =>
        openOperatorPickerModal(filterIndex)
      )
    );
  } else if (filter.left) {
    rowDiv.appendChild(
      createButtonToken("? Operator", "builder-token token-op", () =>
        openOperatorPickerModal(filterIndex)
      )
    );
  }
  ...
  if (filter.right)
    ...
    rowDiv.appendChild(
      createButtonToken("📊 Select Right", "builder-token", () =>
        openIndicatorPickerModal(filterIndex, "right")
      )
    );
...
function createOperandToken(operand, side, filterIndex) {
  ...
  if (operand.type === "indicator") {
    const indDef = INDICATORS.find((i) => i.value === operand.value) || {
      label: operand.value,
      params: getImplicitParams(operand.value),
    };
    ...
    const hasConfigurableParams = indDef.params.some((p) =>
      [
        "field",
        "period",
        "fast_period",
    ...
    if (hasConfigurableParams) {
      ...
      let firstParam = true;
      const addComma = () => {
        if (!firstParam) wrapperSpan.appendChild(document.createTextNode(", "));
        firstParam = false;
      };

      if (indDef.params.includes("field")) {
        addComma();
        wrapperSpan.appendChild(
          createButtonToken(
            operand.params.field || "Close",
      ...
      if (indDef.params.includes("period")) {
        addComma();
        wrapperSpan.appendChild(
          createButtonToken(
            String(operand.params.period || 20),
      ...
      if (indDef.params.includes("fast_period")) {
        addComma();
        wrapperSpan.appendChild(
          createButtonToken(
            String(operand.params.fast_period || 12),
      ...
      if (indDef.params.includes("slow_period")) {
        addComma();
        wrapperSpan.appendChild(
          createButtonToken(
            String(operand.params.slow_period || 26),
      ...
      if (indDef.params.includes("signal_period")) {
        addComma();
        wrapperSpan.appendChild(
          createButtonToken(
            String(operand.params.signal_period || 9),
            "token-mini token-per",
            () => openIndicatorConfigEditor(filterIndex, side, "signal_period")
          )
        );
      }
      if (indDef.params.includes("multiplier")) {
        addComma();
        wrapperSpan.appendChild(
          createButtonToken(
            String(operand.params.multiplier || 2),
            "token-mini token-per",
...
function createButtonToken(text, className, onClickHandler) {
  ...
  button.innerHTML = text; // Use innerHTML if text can contain icons like '📊'
  button.addEventListener("click", onClickHandler);
  return button;
}
...
function openIndicatorConfigEditor(filterIndex, targetSide, focusField = null) {
  ...
  indicatorConfigModalLabel.textContent = `Configure ${
    currentOperand.label || currentOperand.value
  }`;
  indicatorConfigTimeframeSelect.value =
    currentOperand.params.timeframe || "Daily";

  // Show/hide and prefill based on known params for the indicator
  const showIfParam = (paramName, groupElement, inputElement, defaultValue) => {
    if (CURRENT_MODAL_INDICATOR_DEF.params.includes(paramName)) {
      groupElement.style.display = "block";
      inputElement.value = currentOperand.params[paramName] || defaultValue;
    } else {
...
function renderIndicatorModalList() {
  ...
  INDICATORS.filter(
    ...
    li.addEventListener("click", () => {
      ...
      // Reset/default config modal fields for the newly selected indicator
...
function handleIndicatorConfigDone() {
  ...
  if (CURRENT_MODAL_INDICATOR_DEF.params.includes("field"))
    params.field = indicatorConfigFieldSelect.value;
  if (CURRENT_MODAL_INDICATOR_DEF.params.includes("period"))
    params.period = parseInt(indicatorConfigPeriodInput.value, 10);
  if (CURRENT_MODAL_INDICATOR_DEF.params.includes("fast_period"))
    params.fast_period = parseInt(indicatorConfigFastPeriodInput.value, 10);
  if (CURRENT_MODAL_INDICATOR_DEF.params.includes("slow_period"))
    params.slow_period = parseInt(indicatorConfigSlowPeriodInput.value, 10);
  if (CURRENT_MODAL_INDICATOR_DEF.params.includes("signal_period"))
    params.signal_period = parseInt(indicatorConfigSignalPeriodInput.value, 10);
  if (CURRENT_MODAL_INDICATOR_DEF.params.includes("multiplier"))
    params.multiplier = parseFloat(indicatorConfigMultiplierInput.value);
...
function handleRunScan() {
  ...
  resultsTableBody.innerHTML =
    '<tr><td colspan="6" class="text-center py-5" style="color:#6b7280;"><i class="fas fa-spinner fa-spin me-2"></i>Loading results...</td></tr>';
  stockCountSpan.textContent = "Matching Stocks: -";

  const transformedFilters = window.filters.map((f) => {
    // Transform filter structure for backend (indicator_utils.py::evaluate_filter_row)
    let transformed = {};
    if (f.left) {
      transformed.indicator = f.left.value; // TALib function name
      ...
      // Add other specific params like fast_period, multiplier if your evaluate_filter_row expects them directly
      if (f.left.params.fast_period)
        transformed.fastperiod = f.left.params.fast_period;
      if (f.left.params.slow_period)
        transformed.slowperiod = f.left.params.slow_period;
      if (f.left.params.signal_period)
        transformed.signalperiod = f.left.params.signal_period;
      if (f.left.params.multiplier)
        transformed.multiplier = f.left.params.multiplier;
    }
    transformed.mainOp = f.op;
    if (f.right) {
      if (f.right.type === "number") {
        ...
        // Assuming right indicator uses same timeframe and field as left for simplicity here.
        // Your evaluate_filter_row might need specific timeframe/field/period for right indicator.
        transformed.rightTimeframe = f.right.params.timeframe;
  ...
  console.log(
    "Transformed filters for backend:",
    JSON.stringify(transformedFilters, null, 2)
  );
  ...
  fetch("/api/run_screener/", {
    ...
    .then((data) => {
      // Your run_screener returns { "matches": ["SYMBOL1", "SYMBOL2"] }
      // updateResultsTable needs more details. For now, just list symbols.
      // To get full details, backend needs to provide them or another API call is needed.
      if (data.matches) {
        const resultsForTable = data.matches.map((symbol) => ({
          symbol: symbol,
          ltp: "N/A",
          change_pct: "N/A",
          volume: "N/A",
        }));
        updateResultsTable(resultsForTable);
      } else {
        console.error("Scan response did not contain 'matches' array:", data);
        updateResultsTable([]);
      }
    })
    .catch((error) => {
      console.error("Error running scan:", error);
      let errorMsg = "Error running scan.";
      if (error && typeof error === "object" && error.error)
        errorMsg = error.error;
      else if (typeof error === "string") errorMsg = error;
      resultsTableBody.innerHTML = `<tr><td colspan="6" class="text-center py-5" style="color:#f87171;">${errorMsg}</td></tr>`;
...
function updateResultsTable(results) {
  const tbody = document.getElementById("resultsTableBody");
  const noResultsRow = document.getElementById("noResultsRow"); // Assumes this row exists in HTML
  tbody.innerHTML = "";

  if (!results || results.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="6" class="text-center py-5" style="color:#6b7280;">No stocks match your criteria.</td></tr>';
    document.getElementById("stockCount").textContent = "Matching Stocks: 0";
    return;
  }
  ...
  results.forEach((stock, index) => {
    const tr = tbody.insertRow();
    // LTP and Change % styling needs actual numeric values for comparison
    let changePctStyle = "";
