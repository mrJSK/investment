...
# def process_financial_corporate_actions():
#     """
#     REVISED: Fetches and processes the dedicated Corporate Actions feed
#     for dividends, splits, etc., and saves them to the database with more detail.
#     """
#     rss_url = "https://nsearchives.nseindia.com/content/RSS/Corporate_action.xml"
#     print(f"Fetching Financial Corporate Actions from: {rss_url}")
#     try:
#         feed = feedparser.parse(rss_url)
#         if feed.bozo:
#             print(f"Warning: Malformed Financial Actions feed. Reason: {feed.bozo_exception}")
#     except Exception as e:
#         print(f"❌ ERROR fetching financial corporate actions: {e}")
#         return
...
#     for entry in feed.entries:
#         try:
#             # --- 1. Parse the description field into a dictionary ---
#             desc = entry.get("description", "")
#             details_dict = {item.split(':')[0].strip(): item.split(':')[1].strip() for item in desc.split('|') if ':' in item}
            
#             # --- 2. Clean up company name from the title ---
#             company_name = entry.title.split(' - Ex-Date:')[0].strip()

#             # --- 3. Determine a more specific action type from PURPOSE ---
#             purpose = details_dict.get('PURPOSE', '').upper()
#             action_type = "Financial" # Default type
#             if 'DIVIDEND' in purpose:
#                 action_type = 'Dividend'
#                 # Use regex to find the specific dividend amount text
#                 dividend_match = re.search(r'RS\.?\s*[\d\.]+\s*PER\s*SHARE', purpose)
#                 if dividend_match:
#                     details_dict['DividendAmount'] = dividend_match.group(0)
...
#             record_date_str = details_dict.get('RECORD DATE')
#             # Check for placeholder value before parsing
#             if record_date_str and record_date_str != '-':
#                 record_date = datetime.strptime(record_date_str, '%d-%b-%Y').date()
#             else:
#                 record_date = None

#             # --- 5. Save the detailed data to the database ---
#             CorporateAction.objects.update_or_create(
#                 link=entry.link, # Using link as a unique identifier for the event
#                 subject=entry.title,
#                 defaults={
#                     'company_name': company_name,
#                     'category': 'Financial',
...
#                     'ex_date': ex_date,
#                     'record_date': record_date,
#                     'details': details_dict, # Save the rich details dictionary
#                 }
#             )
#             print(f"   -> Saved Financial Event: [{action_type}] for {company_name}")
...
#     try:
#         # Run both helper functions to update the database from the two RSS feeds
...
#     # Fetch the 15 most recent actions from the database, showing Financial ones first
#     actions = CorporateAction.objects.order_by('category', '-scraped_at')[:15]
    
#     # Safely create a list of dictionaries for the JSON response
#     fresh_data = []
#     for a in actions:
#         fresh_data.append({
...
#             "link": a.link,
#             "details": a.details,
#             # Safely check for attributes that might not exist on all records
#             "publication_date": getattr(a, 'publication_date_str', None),
#             "ex_date": a.ex_date
#         })
...
# --- Database and Cache helpers wrapped for async usage ---
...
def db_get_announcements():
    all_announcements = ImportantAnnouncement.objects.order_by('priority', '-scraped_at')[:200]
    categorized = defaultdict(list)
    for ann in all_announcements:
        categorized[ann.category].append({
            "company": ann.company_name, "description": ann.description,
            "link": ann.link, "pub_date": ann.publication_date_str
        })
    return dict(categorized)

@sync_to_async
def db_get_annual_reports():
    reports = FinancialReport.objects.order_by('-report_date')[:10]
    return [report.structured_data for report in reports]

@sync_to_async
def db_get_quarterly_reports():
    reports = QuarterlyFinancials.objects.order_by('-period_end_date').distinct('company_name')[:10]
    return [report.structured_data for report in reports]

@sync_to_async
def db_get_corporate_actions():
    actions = CorporateAction.objects.order_by('-scraped_at')[:20]
    return [{"company_name": a.company_name, "action_type": a.action_type, "category": a.category, "link": a.link} for a in actions]
...
# --- ASYNCHRONOUS API-FACING SERVICE FUNCTIONS ---
# These functions are now extremely fast because they only read from the cache or local database.
# The slow web scraping should be handled by a separate background task (like your Celery task).

async def get_and_update_market_news():
    return await db_get_news()

async def get_cached_or_fresh_announcements():
    CACHE_KEY = 'nse_announcements_data_prioritized'
    cached_data = await cache_get(CACHE_KEY)
    if cached_data:
        return cached_data
    fresh_data = await db_get_announcements()
    await cache_set(CACHE_KEY, fresh_data, 300)
    return fresh_data

async def get_cached_or_fresh_financial_reports():
    CACHE_KEY = 'financial_reports_data'
    cached_data = await cache_get(CACHE_KEY)
    if cached_data:
        return cached_data
    fresh_data = await db_get_annual_reports()
    await cache_set(CACHE_KEY, fresh_data, 3600)
    return fresh_data

async def get_latest_quarterly_reports():
    CACHE_KEY = 'quarterly_reports_data'
    cached_data = await cache_get(CACHE_KEY)
    if cached_data:
        return cached_data
    fresh_data = await db_get_quarterly_reports()
    await cache_set(CACHE_KEY, fresh_data, 3600)
    return fresh_data

async def get_latest_corporate_actions():
    CACHE_KEY = 'corporate_actions_data'
    cached_data = await cache_get(CACHE_KEY)
    if cached_data:
        return cached_data
    fresh_data = await db_get_corporate_actions()
    await cache_set(CACHE_KEY, fresh_data, 300)
    return fresh_data

# NOTE: The sentiment analysis model and all the slow scraping functions 
# (e.g., process_financial_corporate_actions, scrape_and_store_news) 
# are intentionally removed from this file. Their logic should be placed 
# inside your `dashboard/tasks.py` file to be run only by Celery.